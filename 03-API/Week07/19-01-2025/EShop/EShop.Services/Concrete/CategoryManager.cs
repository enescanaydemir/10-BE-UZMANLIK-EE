...
namespace EShop.Services.Concrete
{
    public class CategoryManager : ICategoryService
    {
        ...
        public CategoryManager(IUnitOfWork unitOfWork, IMapper mapper, IImageService imageService)
        {
            _unitOfWork = unitOfWork;
            _mapper = mapper;
            _categoryRepository = _unitOfWork.GetRepository<Category>();
            _imageManager = imageService;
        }

        public async Task<ResponseDto<CategoryDto>> AddAsync(CategoryCreateDto categoryCreateDto) //Kategori ekleme işlemi
        {
            try
            {
        ...
        public async Task<ResponseDto<NoContent>> SoftDeleteAsync(int id)
        {
            try
            {
                ...
                if (result < 1)
                {
                    return ResponseDto<NoContent>.Fail("Kategori silinmeye ya da geri alınmaya çalışırken bir hata oluştu!", StatusCodes.Status500InternalServerError);
                }
        ...
        public async Task<ResponseDto<NoContent>> UpdateAsync(CategoryUpdateDto categoryUpdateDto)
        {
            try
            {
                ...
                {
                    return ResponseDto<NoContent>.Fail("Pasif kategoriler güncellenemez! Önce güncellemek istediğiniz kategoriyi Aktif duruma getirmeniz gerekir!", StatusCodes.Status400BadRequest);
                }
                ...
                //Resim operasyonu
                if (categoryUpdateDto.Image != null) // null değil ise resim yükle
                {
                    var imageResponse = await _imageManager.UploadImageAsync(categoryUpdateDto.Image);
                    if (!imageResponse.IsSuccessful) // eğer resim yükleme işlemi başarısız ise
                    {
                        return ResponseDto<NoContent>.Fail(imageResponse.Error, imageResponse.StatusCode);
                    }
                    _imageManager.DeleteImage(category.ImageUrl); // Eski resmi sil
                    category.ImageUrl = imageResponse.Data ?? "/images/default-category.png";
                }
                _mapper.Map(categoryUpdateDto, category); // AutoMapper ile gelen verileri map ediyoruz. Güncelleme yapılıyor.
                _categoryRepository.Update(category);
                var result = await _unitOfWork.SaveAsync();
                if (result < 1)
                {
